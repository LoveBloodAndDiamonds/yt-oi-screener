...
class SettingsModelView(_CustomModelView):
    fields = [
        IntegerField(
            "interval",
            label="Интервал измерения роста открытого интереса, сек.",
            help_text="За какое время измерять открытый интерес, в секундах.",
            required=True,
        ),
        FloatField(
            "min_growth_prct",
            label="Минимальный рост открытого интереса, %",
            help_text="Минимальный процент роста открытого интереса за указанный интервал.",
            required=True,
        ),
        IntegerField(
            "timeout",
